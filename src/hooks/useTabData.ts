...
export function useTabData(tabId: TabId) {
  const [prevTabId, setPrevTabId] = useState<TabId>(tabId);
  const [sections, setSections] = useState<Section[]>(() => tabCache.get(tabId) ?? []);
  const [loading, setLoading] = useState<boolean>(!tabCache.has(tabId));

  // Sync state if tabId changes during rendering
  if (tabId !== prevTabId) {
    setPrevTabId(tabId);
    setSections(tabCache.get(tabId) ?? []);
    setLoading(!tabCache.has(tabId));
  }
